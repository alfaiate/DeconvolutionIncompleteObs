function [ rmse, elapsed, estimated_image, estimated_image_full ] = deblur_AM_color( observed_image, blur_kernel, mask, maxiter, lambda, miu_1, miu_2, final_image, min_error, show_image, print_status, compute_quality )
%deblur_AM_color - Corresponds to the AM method discussed in [1],
...
%           = deblur_AM_color( observed_image, blur_kernel, mask, maxiter, lambda, miu_1, 
...
% miu_2: another penalization parameter
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% I. It starts by initializing the variables used by AM.
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
%     Primal
    rf = miu_1 * fft2([csi1(end,:,:) - csi1(1,:,:); -diff(csi1,1,1)] + ...
        [csi2(:,end,:) - csi2(:,1,:), -diff(csi2,1,2)]) + miu_2 * hfc .* fft2(csi3);
    xf = rf ./ (miu_1 * gf2sum + miu_2 * hf2);
    x = real(ifft2(xf));
...
%     Soft-threshold (isotropic TV)
    ...
    s3 = real(ifft2(hf .* xf)) - d3;
    v3 = (Mty + miu_2 * s3) ./ (MtM + miu_2);
