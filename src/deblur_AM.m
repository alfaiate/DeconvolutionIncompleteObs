function [ rmse, elapsed, snr, isnr, ssim, estimated_image, estimated_image_full ] = deblur_AM( observed_image, blur_kernel, mask, maxiter, lambda, miu_1, miu_2, original_image, final_image, min_error, show_image, print_status, compute_quality )
%deblur_AM - Corresponds to the AM method discussed in [1]
% 
...
%     Primal
    rf = miu_1 * fft2([csi(end,:,1) - csi(1,:,1); -diff(csi(:,:,1),1,1)] + ...
        [csi(:,end,2) - csi(:,1,2), -diff(csi(:,:,2),1,2)]) + miu_2 * hfc .* fft2(csi(:,:,3));
...
%     Soft-threshold (isotropic TV)
    ...
    s3 = real(ifft2(hf .* xf)) - d(:,:,3);
    v(:,:,3) = (Mty + miu_2 * s3) ./ (MtM + miu_2);
